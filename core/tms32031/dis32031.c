...
void append_indirect(UINT8 ma, INT8 disp, char *buffer, int flags)
{
	...
	dispstr[0] = 0;
	if (disp < 0)
		sprintf(dispstr, "-0x%X", -disp & 0xffff);
	else if (disp > 0)
		sprintf(dispstr, "0x%X", disp);

	switch (mode)
	{
		case 0:		sprintf(dst, "RD%s(AR%d + %s)", (flags & FLOAT) ? "F" : "", ar, dispstr);	break;
		case 1:		sprintf(dst, "RD%s(AR%d - %s)", (flags & FLOAT) ? "F" : "", ar, dispstr);	break;
		case 2:		sprintf(dst, "RD%s(AR%d += %s)", (flags & FLOAT) ? "F" : "", ar, dispstr);	break;
		case 3:		sprintf(dst, "RD%s(AR%d -= %s)", (flags & FLOAT) ? "F" : "", ar, dispstr);	break;
		case 4:		sprintf(dst, "RD%s(AR%d) (postmod +%s)", (flags & FLOAT) ? "F" : "", ar, dispstr);	break;
		case 5:		sprintf(dst, "RD%s(AR%d) (postmod -%s)", (flags & FLOAT) ? "F" : "", ar, dispstr);	break;
		case 6:		sprintf(dst, "RD%s(AR%d) (postmod +%s)%%", (flags & FLOAT) ? "F" : "", ar, dispstr);	break;
		case 7:		sprintf(dst, "RD%s(AR%d) (postmod -%s)%%", (flags & FLOAT) ? "F" : "", ar, dispstr);	break;

		case 8:		sprintf(dst, "RD%s(AR%d + IR0)", (flags & FLOAT) ? "F" : "", ar);	break;
		case 9:		sprintf(dst, "RD%s(AR%d - IR0)", (flags & FLOAT) ? "F" : "", ar);	break;
		case 10:	sprintf(dst, "RD%s(AR%d += IR0)", (flags & FLOAT) ? "F" : "", ar);	break;
		case 11:	sprintf(dst, "RD%s(AR%d -= IR0)", (flags & FLOAT) ? "F" : "", ar);	break;
		case 12:	sprintf(dst, "RD%s(AR%d) (postmod +IR0)", (flags & FLOAT) ? "F" : "", ar);	break;
		case 13:	sprintf(dst, "RD%s(AR%d) (postmod -IR0)", (flags & FLOAT) ? "F" : "", ar);	break;
		case 14:	sprintf(dst, "RD%s(AR%d) (postmod +IR0)%%", (flags & FLOAT) ? "F" : "", ar);	break;
		case 15:	sprintf(dst, "RD%s(AR%d) (postmod -IR0)%%", (flags & FLOAT) ? "F" : "", ar);	break;

		case 16:	sprintf(dst, "RD%s(AR%d + IR0)", (flags & FLOAT) ? "F" : "", ar);	break;
		case 17:	sprintf(dst, "RD%s(AR%d - IR0)", (flags & FLOAT) ? "F" : "", ar);	break;
		case 18:	sprintf(dst, "RD%s(AR%d += IR1)", (flags & FLOAT) ? "F" : "", ar);	break;
		case 19:	sprintf(dst, "RD%s(AR%d -= IR1)", (flags & FLOAT) ? "F" : "", ar);	break;
		case 20:	sprintf(dst, "RD%s(AR%d) (postmod +IR1)", (flags & FLOAT) ? "F" : "", ar);	break;
		case 21:	sprintf(dst, "RD%s(AR%d) (postmod -IR1)", (flags & FLOAT) ? "F" : "", ar);	break;
		case 22:	sprintf(dst, "RD%s(AR%d) (postmod +IR1)%%", (flags & FLOAT) ? "F" : "", ar);	break;
		case 23:	sprintf(dst, "RD%s(AR%d) (postmod -IR1)%%", (flags & FLOAT) ? "F" : "", ar);	break;

		case 24:	sprintf(dst, "RD%s(AR%d)", (flags & FLOAT) ? "F" : "", ar);			break;
...
static void append_immediate(UINT16 data, int is_float, int is_unsigned, char *buffer)
{
	...
	if (is_float)
	{
		...
		sprintf(dst, "-0x%04X", -data & 0xffff);
	else
		sprintf(dst, "0x%04X", data);
}
...
static void disasm_general(const char *opstring, UINT32 op, int flags, char *buffer)
{
	...
	if (!(flags & NODEST) && !(flags & SWAPSRCDST))
	{
		...
		if (flags & FLOAT) strcat(buffer, "F");
	}

	strcat(buffer, " ");
	strcat(buffer, opstring);
	strcat(buffer, " ");
	...
	if (!(flags & NOSOURCE))
	{
		switch ((op >> 21) & 3)
		{
			...
			case 1:
				sprintf(&buffer[strlen(buffer)], "RD%s(0x80%04X)", (flags & FLOAT) ? "F" : "", op & 0xffff);
				break;

			case 2:
				append_indirect((op >> 8) & 0xff, op, buffer, flags);
				break;
...
static void disasm_3op(const char *opstring, UINT32 op, int flags, char *buffer)
{
	...
	if (!(flags & NOSOURCE1))
	{
		switch ((op >> 21) & 1)
		{
			...
			case 1:
				append_indirect(op >> 8, 1, buffer, flags);
				break;
		}
	}
	...
	if (!(flags & NOSOURCE2))
	{
		switch ((op >> 22) & 1)
		{
			...
			case 1:
				append_indirect(op, 1, buffer, flags);
				break;
		}
	}
}
...
static void disasm_parallel_3op3op(const char *opstring1, const char *opstring2, UINT32 op, int flags, const UINT8 *srctable, char *buffer)
{
	...
	src[3][0] = 0;
	append_indirect(op >> 8, 1, src[3], flags);

	src[4][0] = 0;
	append_indirect(op, 1, src[4], flags);

	sprintf(buffer, "R%d%s = %s%s %s %s%s || R%d%s = %s%s %s %s%s",
			d1, (flags & FLOAT) ? "F" : "", src[s[0]], (flags & FLOAT) ? "F" : "", opstring1, src[s[1]], (flags & FLOAT) ? "F" : "", 
			d2, (flags & FLOAT) ? "F" : "", src[s[2]], (flags & FLOAT) ? "F" : "", opstring2, src[s[3]], (flags & FLOAT) ? "F" : "");
}
...
static void disasm_parallel_3opstore(const char *opstring1, const char *opstring2, UINT32 op, int flags, char *buffer)
{
	...
	dst2[0] = 0;
	append_indirect(op >> 8, 1, dst2, flags);

	src2[0] = 0;
	append_indirect(op, 1, src2, flags);

	if (!(flags & NOSOURCE1))
		sprintf(buffer, "R%d%s = R%d%s %s %s%s || %s %s,R%d%s",
				d1, (flags & FLOAT) ? "F" : "", s1, (flags & FLOAT) ? "F" : "", opstring1, src2, (flags & FLOAT) ? "F" : "",
				opstring2, dst2, s3, (flags & FLOAT) ? "F" : "");
	else
		sprintf(buffer, "R%d%s = %s %s%s || %s %s,R%d%s",
				d1, (flags & FLOAT) ? "F" : "", opstring1, src2, (flags & FLOAT) ? "F" : "",
				opstring2, dst2, s3, (flags & FLOAT) ? "F" : "");
}
...
static void disasm_parallel_loadload(const char *opstring1, const char *opstring2, UINT32 op, int flags, char *buffer)
{
	...
	src1[0] = 0;
	append_indirect(op >> 8, 1, src1, flags);

	src2[0] = 0;
	append_indirect(op, 1, src2, flags);

	sprintf(buffer, "R%d%s = %s || R%d%s = %s",
			d2, (flags & FLOAT) ? "F" : "", src2,
			d1, (flags & FLOAT) ? "F" : "", src1);
}
...
static void disasm_parallel_storestore(const char *opstring1, const char *opstring2, UINT32 op, int flags, char *buffer)
{
	...
	dst1[0] = 0;
	append_indirect(op >> 8, 1, dst1, flags);

	dst2[0] = 0;
	append_indirect(op, 1, dst2, flags);
...
unsigned dasm_tms32031(char *buffer, unsigned pc)
{
	...
	switch (op >> 23)
	{
		case 0x000:	disasm_general("fabs(", op, FLOAT, buffer);			break;
		case 0x001:	disasm_general("labs(", op, INTEGER, buffer);		break;
		case 0x002:	disasm_general("+=carry", op, INTEGER, buffer);		break;
		case 0x003:	disasm_general("+=", op, FLOAT, buffer);			break;
		case 0x004:	disasm_general("+=", op, INTEGER, buffer);		break;
		case 0x005:	disasm_general("&=", op, INTEGER | UNSIGNED, buffer);	break;
		case 0x006:	disasm_general("&= ~", op, INTEGER | UNSIGNED, buffer);	break;
		case 0x007:	disasm_general("<<=arith", op, INTEGER, buffer);			break;

		case 0x008:	disasm_general("CMPF", op, FLOAT, buffer);			break;
		case 0x009:	disasm_general("CMPI", op, INTEGER, buffer);		break;
		case 0x00a:	disasm_general("= (int)", op, FLOAT, buffer);			break;
		...
		case 0x00e:	disasm_general("=", op, FLOAT, buffer);			break;
		case 0x00f:	disasm_general("LDFI", op, FLOAT, buffer);			break;

		case 0x010:	disasm_general("=", op, INTEGER, buffer);			break;
		case 0x011:	disasm_general("LDII", op, INTEGER, buffer);		break;
		case 0x012:	disasm_general("LDM", op, FLOAT, buffer);			break;
		case 0x013:	disasm_general("<<=", op, INTEGER, buffer);			break;
		case 0x014:	disasm_general("*=", op, FLOAT, buffer);			break;
		case 0x015:	disasm_general("*=", op, INTEGER, buffer);		break;
		case 0x016:	disasm_general("= -", op, INTEGER, buffer);		break;
		case 0x017:	disasm_general("= -", op, FLOAT, buffer);			break;

		case 0x018:	disasm_general("= -", op, INTEGER, buffer);		break;
		case 0x019:	disasm_general("NOP", op, NODEST, buffer); 			break;
		case 0x01a:	disasm_general("NORM", op, FLOAT, buffer);			break;
		case 0x01b:	disasm_general("= ~", op, INTEGER, buffer);			break;
		...
		case 0x020:	disasm_general("|=", op, INTEGER | UNSIGNED, buffer);	break;
		...
		case 0x023:	disasm_general("<<rotate", op, INTEGER, buffer);			break;
		case 0x024:	disasm_general("<<rotcarry", op, INTEGER, buffer);		break;
		case 0x025:	disasm_general(">>rotate", op, INTEGER, buffer);			break;
		case 0x026:	disasm_general(">>rotcarry", op, INTEGER, buffer);		break;
		...
		case 0x02e:	disasm_general("-=carry", op, INTEGER, buffer);		break;
		case 0x02f:	disasm_general("-=", op, FLOAT, buffer);			break;

		case 0x030:	disasm_general("-=", op, INTEGER, buffer);		break;
		case 0x031:	disasm_general("SUBRB", op, INTEGER, buffer);		break;
		case 0x032:	disasm_general("-=reverse", op, FLOAT, buffer);			break;
		case 0x033:	disasm_general("-=reverse", op, INTEGER, buffer);		break;
		case 0x034:	disasm_general("test", op, INTEGER, buffer);		break;
		case 0x035:	disasm_general("^=", op, INTEGER | UNSIGNED, buffer);	break;
		case 0x036:	disasm_general("IACK", op, INTEGER | NODEST, buffer); break;

		case 0x040:	disasm_3op("+carry", op, INTEGER, buffer);			break;
		case 0x041:	disasm_3op("+", op, FLOAT, buffer);				break;
		case 0x042:	disasm_3op("+", op, INTEGER, buffer);			break;
		case 0x043:	disasm_3op("&", op, INTEGER, buffer);			break;
		case 0x044:	disasm_3op("& ~", op, INTEGER, buffer);			break;
		case 0x045:	disasm_3op("<<arith", op, INTEGER, buffer);			break;
		case 0x046:	disasm_3op("CMPF3", op, FLOAT | NODEST, buffer);	break;
		case 0x047:	disasm_3op("CMPI3", op, INTEGER | NODEST, buffer);	break;

		case 0x048:	disasm_3op("<<", op, INTEGER, buffer);			break;
		case 0x049:	disasm_3op("*", op, FLOAT, buffer);				break;
		case 0x04a:	disasm_3op("*", op, INTEGER, buffer);			break;
		case 0x04b:	disasm_3op("|", op, INTEGER, buffer);				break;
		case 0x04c:	disasm_3op("-borrow", op, INTEGER, buffer);			break;
		case 0x04d:	disasm_3op("-", op, FLOAT, buffer);				break;
		case 0x04e:	disasm_3op("-", op, INTEGER, buffer);			break;
		case 0x04f:	disasm_3op("test", op, INTEGER, buffer);			break;

		case 0x050:	disasm_3op("^", op, INTEGER, buffer);			break;
		...
		case 0x0c0: case 0x0c1:
			sprintf(buffer, "goto _%06X", op & 0xffffff);
			break;

		case 0x0c2: case 0x0c3:
			sprintf(buffer, "goto_delayed   _%06X", op & 0xffffff);
			break;

		case 0x0c4: case 0x0c5:
			sprintf(buffer, "_%06X()", op & 0xffffff);
			break;


		case 0x0c8: case 0x0c9:
			sprintf(buffer, "RPTB  0x%06X", op & 0xffffff);
			break;
		...
		case 0x0d4:
		{
			char temp[10];
			sprintf(temp, "B%s%s", condition[(op >> 16) & 31], ((op >> 21) & 1) ? "D" : "");
			sprintf(buffer, "%-6s_%06X", temp, (pc + (((op >> 21) & 1) ? 3 : 1) + (INT16)op) & 0xffffff);
			break;
		}
		...
		case 0x0dc: case 0x0dd: case 0x0de: case 0x0df:
		{
			char temp[10];
			sprintf(temp, "DB%s%s", condition[(op >> 16) & 31], ((op >> 21) & 1) ? "D" : "");
			sprintf(buffer, "%-6sAR%d,0x%06X", temp, (op >> 22) & 7, (pc + (((op >> 21) & 1) ? 3 : 1) + (INT16)op) & 0xffffff);
			break;
		}
		...
		case 0x0e4:
		{
			char temp[10];
			sprintf(temp, "CALL%s", condition[(op >> 16) & 31]);
			sprintf(buffer, "%-6s_%06X", temp, (pc + 1 + (INT16)op) & 0xffffff);
			break;
		}
		...
		case 0x0e8: case 0x0e9: case 0x0ea: case 0x0eb:
		{
			...
			sprintf(buffer, "%-6s0x%02X", temp, op & 31);
			break;
		}
		...
		case 0x104: case 0x105:	case 0x106: case 0x107:	// MPYF3||ADDF3
		{
			...
			disasm_parallel_3op3op("*", "+", op, FLOAT, srctable, buffer);
			break;
		}
		...
		{
			static const UINT8 srctable[] = { 3,4,1,2, 3,1,4,2, 1,2,3,4, 3,1,2,4 };
			disasm_parallel_3op3op("*", "+", op, INTEGER, srctable, buffer);
			break;
		}
		...
		case 0x190: case 0x191:	case 0x192: case 0x193:	// ABSF||STF
			disasm_parallel_3opstore("fabs", "STF", op, FLOAT | NOSOURCE1, buffer);
			break;

		case 0x194: case 0x195:	case 0x196: case 0x197:	// ABSI||STI
			disasm_parallel_3opstore("labs", "STI", op, INTEGER | NOSOURCE1, buffer);
			break;


		case 0x198: case 0x199:	case 0x19a: case 0x19b:	// ADDF3||STF
			disasm_parallel_3opstore("+", "STF", op, FLOAT, buffer);
			break;

		case 0x19c: case 0x19d:	case 0x19e: case 0x19f:	// ADDI3||STI
			disasm_parallel_3opstore("+", "STI", op, INTEGER, buffer);
			break;
		...
		case 0x1a4: case 0x1a5:	case 0x1a6: case 0x1a7:	// ASH3||STI
			disasm_parallel_3opstore("<<arith", "STI", op, INTEGER, buffer);
			break;


		case 0x1a8: case 0x1a9:	case 0x1aa: case 0x1ab:	// FIX||STI
			disasm_parallel_3opstore("(int)", "STF", op, FLOAT | NOSOURCE1, buffer);
			break;
		...
		case 0x1c4: case 0x1c5:	case 0x1c6: case 0x1c7:	// NEGF||STF
			disasm_parallel_3opstore("-", "STF", op, FLOAT | NOSOURCE1, buffer);
			break;


		case 0x1c8: case 0x1c9:	case 0x1ca: case 0x1cb:	// NEGI||STI
			disasm_parallel_3opstore("-", "STI", op, INTEGER | NOSOURCE1, buffer);
			break;
