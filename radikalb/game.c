...
void GameInit(GameSavedData *data)
{
	...
	for (y = 0; y < TEXTURE_MASK_SIZE*8 / 4096; y++)
		for (x = 0; x < 4096; x++)
		{
			...
			if (romTexture[8 + moffshi][moffslo] & (1 << (x % 8)))
	...
	for (i = 0; i < 0x80000; i++)
	{
		g68000MemoryBase[i*4+0] = rom68020[0][i];
		g68000MemoryBase[i*4+1] = rom68020[1][i];
		g68000MemoryBase[i*4+2] = rom68020[2][i];
		g68000MemoryBase[i*4+3] = rom68020[3][i];
	}
	...
	// initialize the CPUs
	InitCPU(0, m68ec020_get_info, &g68000CPU);
...
{
	int startCycles68000 = 25000000 / 60;
	int startCycles32031 = 50000000 / 60;
...
void UpdateControls(void)
{
	...
	if (GetAsyncKeyState(VK_DOWN))
		g68000MemoryBase[0x51000d] ^= 0x08;
	if (GetAsyncKeyState(VK_SPACE))
		g68000MemoryBase[0x51000d] ^= 0x10;
	if (GetAsyncKeyState(VK_LMENU))
		g68000MemoryBase[0x51000d] ^= 0x20;
	if (GetAsyncKeyState(VK_LCONTROL))
		g68000MemoryBase[0x51000d] ^= 0x40;
